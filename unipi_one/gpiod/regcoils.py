...
class RegCoils(virtual.CoilProvider, virtual.RegisterProvider, metaclass=DevMeta):
    ...
    def check(self):
        device = check_dev_type(self.chip, GpioChip, 'Output %s'%self.name, 'chip')
        self.chip = device
        self.chip.register_callback(self.pins, self.event0)
    ...
    async def outputr(self, register, value):
        ...
        try:
            ...
            chip_value = dict(((pin, 1 if (value & (1<<i)) else 0) for i, pin in enumerate(self.pins)))
            async with self.running_lock:
                await self.chip.sendbits(chip_value)
        except Exception as E:
            logging.error(f"Relay {self.name} error: {E.__class__.__name__} {E.args[-1]}")
    ...
    async def outputc(self, coil, values):
        try:
            ...
            chip_value = dict(((self.pins[start+i], value) for i, value in enumerate(values)))
            async with self.running_lock:
                logging.debug(f'Regcoils - set coils {chip_value}')
                await self.chip.sendbits(chip_value)
        except Exception as E:
            logging.error(f"Relay {self.name} error: {E.__class__.__name__} {E.args[-1]}")
    ...
    # hardware -> modbus
    def event0(self, items):
        if hasattr(self,'rdatastore') and hasattr(self,'cdatastore'):
            logging.debug(f'Regcoils - event0 {items}')
            self.rdatastore[self.register] = sum(((1<<i) if items.get(pin,0) else 0 for i, pin in enumerate(self.pins)))
            for i,pin in enumerate(self.pins):
                self.cdatastore[self.start_coil+i] = 1 if (items.get(pin,0)) else 0
            ##self.chip.register_relay(self._mask, self.event)

    def event(self, items):
        logging.debug(f'Regcoils - event {items}')
        self.rdatastore[self.register] = sum(((1<<i) if items.get(pin,0) else 0 for i, pin in enumerate(self.pins)))
        for i,pin in enumerate(self.pins):
            self.cdatastore[self.start_coil+i] = 1 if (items.get(pin,0)) else 0
